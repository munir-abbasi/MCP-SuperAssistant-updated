...
const logger = createLogger('GrokChatInputHandler');
...
export const insertTextToChatInput = (text: string): boolean => {
  try {
    ...
    if (!chatInput) {
      logMessage('Could not find Grok input element');
      logger.error('Could not find Grok input element');
      return false;
    }
    ...
    logger.error('Error inserting text into Grok input:', error);
    return false;
  }
};
...
export const attachFileToChatInput = (file: File): boolean => {
  ...
  try {
    ...
    logger.error('Error attaching file to Grok chat:', error);
    return false;
  }
};
...
export const submitChatInput = async (_maxWaitTime = 5000): Promise<boolean> => {
  try {
    ...
    logger.error('Error submitting Grok chat input:', error);
    return false;
  }
};
