...
export class GrokAdapter extends BaseAdapterPlugin {
  ...
  private readonly selectors = {
    ...
    CHAT_INPUT:
      'textarea#chat-input, textarea[aria-label="Ask Grok anything"], textarea[placeholder="Ask anything"], textarea[placeholder], textarea[spellcheck="false"], div.css-146c3p1 textarea, textarea.r-30o5oe, div[contenteditable="true"]',
    // Submit button selectors (multiple fallbacks)
    SUBMIT_BUTTON:
      'button[aria-label="Submit"], button[aria-label="Send message"], button[aria-label*="Send"], button.send-button, button.chat-submit, button[data-testid="send-button"]',
    // File upload related selectors
    FILE_UPLOAD_BUTTON: 'button[aria-label*="attach"], button[aria-label*="Attach"], button[data-testid="file-upload"]',
    ...
    BUTTON_INSERTION_CONTAINER:
      'div:has(> button[aria-label="Attach file"]), div:has(> button[aria-label="Submit"]), .chat-input-actions, .input-actions, .chat-controls',
  ...
  private emitExecutionCompleted(operation: string, params: Record<string, unknown>, result: unknown): void {
