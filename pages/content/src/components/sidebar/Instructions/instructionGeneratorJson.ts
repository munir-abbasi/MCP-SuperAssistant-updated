...
const getCurrentHost = (): string => window.location.hostname;

const isQwenHost = (hostname: string): boolean => hostname.includes('qwen');

const qwenJsonInstructions = `
[SuperAssistant Qwen JSONL Tool Instructions][IMPORTANT]

You are SuperAssistant inside Qwen Chat. When a tool is needed, output exactly one complete fenced jsonl code block. The browser extension reads that block and provides execution controls.

Strict Qwen output rules:
- Do not write visible planning tags, XML-style reasoning sections, or response-format scaffolding.
- Do not tell the user to run the call.
- Do not add prose before or after a tool-call block once you decide to use a tool.
- Never emit only function_call_start. A valid call always includes function_call_start, all required parameter lines, and function_call_end.
- Invoke only one function per response. After the complete block, stop and wait for the tool result.
- Use only tools listed below. Do not invent tool names or parameters.
- Required parameters must be present as separate JSON Lines events with "type": "parameter", "key", and "value".
- Optional parameters should be omitted unless needed.
- Use exact values from the user when provided.

Required JSON Lines format:
...
export const generateInstructionsJson = (
  ...
  const useQwenInstructions = isQwenHost(currentHost);

  // Add general usage information
  // instructions += '## General Usage\n\n';
  if (useQwenInstructions) {
    instructions += qwenJsonInstructions;
  } else {
    instructions +=
...
`;
  ...
  if (useQwenInstructions) {
    instructions +=
      'QWEN FINAL REMINDER: if a tool is needed, output exactly one complete fenced jsonl block with function_call_start, all required parameter lines, and the matching function_call_end line. Then stop and wait for the extension result. Do not add prose, planning tags, or partial JSONL.\n\n';
  } else {
...
`;
    instructions +=
      'Output the complete jsonl function-call block above and then stop. The extension will detect it and provide execution controls.\n\n';
  }
