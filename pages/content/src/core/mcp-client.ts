import { ContextBridgeDispatchError, contextBridge } from './context-bridge';
...
class McpClient {
  ...
  async callTool(toolName: string, args: Record<string, unknown>, callId?: string): Promise<any> {
    ...
    // Generate execution ID for tracking; carry the renderer's operation identity as correlation
    const executionId = useToolStore.getState().startToolExecution(toolName, args, callId);
    const attemptId = crypto.randomUUID();

    try {
      const timeoutMs = (await import('../stores/ui.store')).useUIStore.getState().preferences.mcpTimeout ?? 60000;

      const result = await contextBridge.sendMessage(
        'background',
        'mcp:call-tool',
        { toolName, args, adapterName, callId, attemptId, timeout: timeoutMs }, // Pass logical operation + concrete attempt identity
        { timeout: timeoutMs + 5000 }, // Add 5s buffer to context bridge timeout
      );

      logMessage(`[McpClient] Tool call successful: ${toolName}`);

      // Update tool execution with success
      useToolStore.getState().completeToolExecution(executionId, result, 'success', undefined, {
      ...
      useToolStore.getState().completeToolExecution(executionId, null, 'error', errorMessage, {
        dispatchState: error instanceof ContextBridgeDispatchError ? error.dispatchState : 'unknown',
        attemptCount: 1,
        attemptId,
      });
