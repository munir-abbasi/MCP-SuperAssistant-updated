...
export class DeepSeekAdapter extends BaseAdapterPlugin {
  ...
  private readonly selectors = {
    ...
    CHAT_INPUT:
      '#chat-input, textarea#chat-input, textarea[id*="input"], textarea[placeholder*="Ask"], textarea[placeholder*="Message DeepSeek"], textarea.chat-input, textarea[spellcheck="false"], div[contenteditable="true"]',
    // Submit button selectors (multiple fallbacks)
    SUBMIT_BUTTON:
      'button[aria-label*="Send"], div[role="button"]:has(svg), button[data-testid="send-button"], button.send-button',
    ...
    BUTTON_INSERTION_CONTAINER:
      'div:has(> button[aria-label*="attach"]), .chat-input-actions, .input-actions, .actions-wrapper, .ec4f5d61',
    // Alternative insertion points
    FALLBACK_INSERTION: '.input-area, .chat-input-container, .bf38813a, .aaff8b8f',
  };
